...
export function CalendarBoard({ installments }: { installments: Installment[] }) {
  ...
  const selectedTotals={previsto:selectedRows.reduce((s,i)=>s+Number(i.amount),0),recebido:selectedRows.reduce((s,i)=>s+Number(i.amount_paid),0),saldo:selectedRows.reduce((s,i)=>s+Number(i.remaining_amount),0)};

  const totals={previsto:monthRows.reduce((s,i)=>s+Number(i.amount),0),recebido:monthRows.reduce((s,i)=>s+Number(i.amount_paid),0),pendente:monthRows.reduce((s,i)=>s+Number(i.remaining_amount),0),atrasado:monthRows.filter(i=>effectiveInstallmentStatus(i,todayKey)==="ATRASADO").reduce((s,i)=>s+Number(i.remaining_amount),0)+priorOverdue.reduce((s,i)=>s+Number(i.remaining_amount),0)};
  ...
  return <>
    ...
    <div className="card" style={{marginBottom:16,padding:12}}><div style={{display:"flex",gap:14,flexWrap:"wrap",alignItems:"center",fontSize:12}}><strong>Legenda:</strong><span className="badge green">Pago</span><span className="badge blue">Pendente</span><span className="badge gray">Parcial / reagendado</span><span style={{color:"var(--red)",fontWeight:700}}>● Atrasado</span></div></div>

    {priorOverdue.length>0&&<div className="card" style={{marginBottom:16}}><div className="section-title"><div><h2>Atrasados de meses anteriores</h2><div className="muted" style={{fontSize:12}}>Clique em uma pendência para cobrar, renegociar ou registrar o pagamento.</div></div></div><div className="list">{priorOverdue.map(i=><button type="button" className="list-row" key={`prior-${i.id}`} onClick={()=>openDay(i.due_date)} style={{width:"100%",textAlign:"left",cursor:"pointer",border:0}}><div className="person"><div className="avatar">{i.client?.name?.[0]||"?"}</div><div><div className="person-name">{i.client?.name}</div><div className="person-meta">{format(new Date(i.due_date+"T12:00:00"),"dd/MM/yyyy")} · {i.loan?.loan_code} · parcela {i.installment_number}/{i.loan?.installment_count}</div></div></div><div style={{textAlign:"right"}}><strong>{money(i.remaining_amount)}</strong><div style={{marginTop:5}}><StatusBadge status={effectiveInstallmentStatus(i,todayKey)}/></div></div></button>)}</div></div>}

    <div className="card">
      <div className="section-title"><div><h2 style={{textTransform:"capitalize"}}>{format(month,"MMMM 'de' yyyy",{locale:ptBR})}</h2><div className="muted" style={{fontSize:12}}>Clique em qualquer dia para ver e atualizar os recebimentos.</div></div><div style={{display:"flex",gap:7}}><button className="btn secondary" onClick={()=>setMonth(new Date())}>Hoje</button><button className="icon-btn" onClick={()=>setMonth(subMonths(month,1))}><ChevronLeft size={17}/></button><button className="icon-btn" onClick={()=>setMonth(addMonths(month,1))}><ChevronRight size={17}/></button></div></div>
      <div className="calendar-grid">{["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"].map(d=><div key={d} className="calendar-head">{d}</div>)}{days.map(day=>{const key=format(day,"yyyy-MM-dd");const rows=installments.filter(i=>i.due_date===key);return <button type="button" key={key} className="calendar-day" onClick={()=>openDay(key)} style={{opacity:isSameMonth(day,month)?1:.35,cursor:"pointer",textAlign:"left",border:key===todayKey?"1px solid var(--accent)":"inherit",font:"inherit",color:"inherit"}}><div className="day-number">{format(day,"d")}</div>{rows.slice(0,4).map(i=>{const status=effectiveInstallmentStatus(i,todayKey);return <div className={`calendar-item ${status}`} key={i.id} title={`${i.client?.name} ${money(i.remaining_amount)}`}>{i.client?.name} · {money(i.remaining_amount)}</div>})}{rows.length>4&&<div className="muted" style={{fontSize:10,marginTop:5}}>+{rows.length-4} recebimentos</div>}</button>})}</div>
      <div className="list" style={{marginTop:10}}>{[...monthRows].sort((a,b)=>a.due_date.localeCompare(b.due_date)).map(i=><button type="button" className="list-row" key={i.id} onClick={()=>openDay(i.due_date)} style={{width:"100%",textAlign:"left",cursor:"pointer",border:0}}><div className="person"><div className="avatar">{i.client?.name?.[0]||"?"}</div><div><div className="person-name">{i.client?.name}</div><div className="person-meta">{format(new Date(i.due_date+"T12:00:00"),"dd/MM/yyyy")} · {i.loan?.loan_code} · parcela {i.installment_number}/{i.loan?.installment_count}</div></div></div><div style={{textAlign:"right"}}><strong>{money(i.remaining_amount)}</strong><div style={{marginTop:5}}><StatusBadge status={effectiveInstallmentStatus(i,todayKey)}/></div></div></button>)}</div>
    </div>

    {selectedDate&&<div className="modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)setSelectedDate(null)}}><div className="modal" style={{maxWidth:820}}>
      ...
      <div className="kpi-strip" style={{marginTop:14}}><div className="kpi-mini"><span className="muted">Previsto</span><strong>{money(selectedTotals.previsto)}</strong></div><div className="kpi-mini"><span className="muted">Recebido</span><strong>{money(selectedTotals.recebido)}</strong></div><div className="kpi-mini"><span className="muted">Saldo</span><strong>{money(selectedTotals.saldo)}</strong></div></div>
      {error&&<div className="alert" style={{marginTop:14}}>{error}</div>}{message&&<div className="alert" style={{marginTop:14}}>{message}</div>}
      ...
      <div className="list" style={{marginTop:14}}>{selectedRows.map(i=>{const status=effectiveInstallmentStatus(i,todayKey);const hasPaid=Number(i.amount_paid)>0;const remaining=Number(i.remaining_amount);return <div className="card" key={`manage-${i.id}`} style={{padding:14}}>
