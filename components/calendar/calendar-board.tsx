...
export function CalendarBoard({ installments }: { installments: Installment[] }) {
  ...
  const monthRows=installments.filter(i=>i.due_date.startsWith(format(month,"yyyy-MM")));
  const totals={
    ...
    atrasado:monthRows.filter(i=>effectiveInstallmentStatus(i)==="ATRASADO").reduce((s,i)=>s+Number(i.remaining_amount),0),
  };
  return <>
