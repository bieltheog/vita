...
export function LoanList({loans,installments}:{loans:Loan[];installments:Installment[]}){
  ...
  return <div className="card">
    ...
    <div className="table-wrap"><table><thead><tr><th>Código</th><th>Cliente</th><th>Emprestado</th><th>Lucro</th><th>Total</th><th>Forma</th><th>Parcelas</th><th>Status</th><th>Ações</th></tr></thead><tbody>{rows.map(x=><tr key={x.id}><td><strong>{x.loan_code}</strong></td><td>{x.client?.name}</td><td>{money(x.principal_amount)}</td><td>{money(x.expected_profit)}</td><td>{money(x.total_receivable)}</td><td>{labels[x.payment_frequency]||x.payment_frequency}</td><td>{x.installment_count}</td><td><span className={`badge ${x.status==='ATIVO'?'green':'gray'}`}>{x.status}</span></td><td><div style={{display:"flex",gap:6}}><EditLoanForm loan={x} installments={installments.filter(row=>row.loan_id===x.id)}/><Link className="icon-btn" href={`/emprestimos/${x.id}`} title="Histórico e parcelas"><History size={15}/></Link></div></td></tr>)}</tbody></table></div>
