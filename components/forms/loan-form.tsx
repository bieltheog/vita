...
export function LoanForm({ clients, demo=false }: { clients: Client[]; demo?: boolean }) {
  ...
  const [principal,setPrincipal]=useState(1000); const [returnValue,setReturnValue]=useState(30); const [type,setType]=useState<"percentage"|"fixed">("percentage"); const [count,setCount]=useState(5);
  ...
  async function submit(formData:FormData){ setError(""); start(async()=>{try{await createLoanAction(formData);setOpen(false)}catch(e){setError(e instanceof Error?e.message:"Erro ao criar empréstimo")}})}
  return <>
    ...
    {open&&<div className="modal-backdrop"><div className="modal">
      ...
      <form action={submit} className="form-grid" style={{marginTop:14}}>
        ...
        <div className="field"><label>Forma de pagamento</label><select className="select" name="payment_frequency"><option value="UNICO">Pagamento único</option><option value="DIARIO">Diário</option><option value="SEMANAL">Semanal</option><option value="QUINZENAL">Quinzenal</option><option value="MENSAL">Mensal</option></select></div>
        ...
        <div className="field"><label>Data do empréstimo *</label><input className="input" name="start_date" type="date" required/></div><div className="field"><label>Primeiro pagamento *</label><input className="input" name="first_due_date" type="date" required/></div>
